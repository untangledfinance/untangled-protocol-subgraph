...
import devDeployments from '../deployments/all_dev.json';
...
const subgraphManifest: any = yaml.load(
  fs.readFileSync(path.resolve('.', 'subgraph.yaml')).toString()
);
...
for (const dataSource of subgraphManifest.dataSources) {
  ...
  if (dataSource.name === 'CurveFiduUSDC') {
  ...
  } else if (dataSource.name === 'LegacyUntangledConfig') {
    dataSource.source.address = localhostContracts['UntangledConfig'].address;
    ...
    dataSource.source.address =
      localhostContracts[dataSource.name as ContractName].address;
  }
}
...
fs.writeFileSync(
  path.resolve(__dirname, '../subgraph-local.yaml'),
  yaml.dump(subgraphManifest, { lineWidth: -1 })
);
