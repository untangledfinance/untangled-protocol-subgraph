...
import {
  getOrInitSeniorPoolStatus,
  recalculateSeniorPoolAPY,
  SENIOR_POOL_STATUS_ID,
} from '../src/entities/senior_pool';
import { handleDepositMade } from '../src/mappings/senior_pool/senior_pool';
import {
  createDepositMadeForSeniorPool,
...
test('recalculateSeniorPoolAPY updates estimatedTotalInterest', () => {
  ...
  seniorPoolStatus.tranchedPools = [
  ...
  let tranchedPool1 = new TranchedPool(
    '0x9999999999999999999999999999999999999999'
  );
  ...
  let tranchedPool2 = new TranchedPool(
    '0x8888888888888888888888888888888888888888'
  );
  ...
  assert.equals(
    ...
    ethereum.Value.fromString(
      seniorPoolStatus.estimatedTotalInterest.toString()
    )
  );
  assert.equals(
...
test('recalculateSeniorPoolAPY updates estimatedApy', () => {
  ...
  seniorPoolStatus.totalPoolAssets = BigInt.fromString(
    '98706336302853217883235969149220777057468050'
  );
  seniorPoolStatus.save();
...
test('handleDepositMade calls recalculateSeniorPoolAPY', () => {
  ...
  const depositMadeEvent = createDepositMadeForSeniorPool(
    seniorPoolAddress,
  ...
  mockUpdateUserCalls(
    ...
    Address.fromString(capitalProviderAddress)
  );
